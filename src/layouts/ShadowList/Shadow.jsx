...
export default function Shadow({ panelNumber, shadow }) {
  ...
  const shadowInputs = shadow.inputs.map((input, index) => {
    if (input.type === "range") {
      return <ShadowRange key={index} inputData={shadow.inputs[index]}
      shadowID={shadow.id}
      />
    }
    ...
    <div>
      <li className="bg-gray-50 border-b border-gray-300">
        ...
        <div className="flex items-end px-6 pt-4">
          ...
          <button className="ml-auto text-sm bg-red-600 text-white hover:bg-red-700 py-1 px-3 rounded">Remove</button>
        </div>
